# 基于Whisper 定制化语音转录与深度文本分析 API 服务

本项目提供一个基于 **微调的 OpenAI Whisper 模型** 的语音转录 API 服务。它具备高效精准的语音到文本转换能力，包括**关键字提取、多维度语义连接词识别、以及智能场景自动判断**，旨在为用户提供从原始音频到结构化、高价值信息的完整解决方案。  
注意测试集请自行到网上搜索下载 **data_thchs30** 本项目支持自动下载**whisper-small模型基座**以及在线训练微调来生成自己的small模型，如其他需要请自行修改代码。
## 项目亮点与核心特性
